 ...
 */
    ...
    class SMC_Object_Type
    {
        ...
        public function init_options($array)
        {
        }
        ...
        public function is_meta_exists($string, $meta)
        {
            $obj	= $this->get($string);
            return array_key_exists($meta, $obj);
        }
        
        public function get_object($id, $t)
        {
            global $new_dir;
            $d										=  $this->get($t);
            $obj									= array();
        ...
        public function get_post_elements($id, $type, $data)
        {
            $el								= get_post($id);
            $obj							= array(
                                                        'post_type'	=> $type,
            ...
            for ($i = 0; $i < count($keys); $i++) {
                ...
                } elseif ($values[$i]['type'] == 'array') {
                    $arr			 		= $this->get_array_property($meta, $values[$i]);
                    if (!is_wp_error($arr)) {
                        $obj[$keys[$i]] = $arr;
                    }
                } elseif ($values[$i]['type'] == 'media') {
                ...
                } elseif ($values[$i]['type'] == 'bool' || $values[$i]['type'] == 'number') {
                    $obj[$keys[$i]]			= (int)$meta;
                } elseif ($values[$i]['type'] == 'string') {
                    $meta					= get_post_meta($id, $keys[$i], true);
                    $obj[$keys[$i]]			= ($meta);
                    if ($values[$i]['download']) {
                    }
        ...
        //
            =================*/
        public function get_taxonomy_element($id, $type, $data)
        {
            if ($data['t']['type'] != "taxonomy") {
                return new WP_Error('no taxonomy');
            }
        ...
        public function get_property($val, $value)
        {
        ...
        public function get_post_property($meta)
        {
            $pp				= get_post($meta);
            if (!$pp) {
                return "";
        ...
        public function get_user_property($meta)
        {
            return "==$meta";
        }
        public function get_taxonomy_property($meta, $object)
        {
            $pp				= get_term_by("id", $meta, $object);
            $pos_slug		= $pp->slug;
            return $pos_slug;
        }
        public function get_db_row_property($meta)
        {
            return $meta;
        }
        public function get_array_property($meta, $values)
        {
            if (!is_array($meta) || 	count($meta)==0) {
                return new WP_Error("parameter is not array");
            }
            if (!is_array($values)) {
                return new WP_Error("SMC_Object element is not array");
            }
            $obj			= array( );
        ...
        */
        public function convert_id($key, $val, $d, $id)
        {
        ...
        public function convert_array($val)
        {
            $arr			= array();
            foreach ($val as $k => $v) {
                if (!$this->get($k)) {
        ...
        public function insert_post_meta($metas, $id, $post_type)
        {
            global $wpdb;
        ...
        public function object_property_args($class_name, $method, $args)
        {
