...
class ShMapperRequest extends SMC_Post
{
    ...
    public static function view_admin_edit($obj)
    {
        ...
        foreach ($bb as $key => $value) {
            if ($key == 't' || $key == 'class' || $key == 'contacts' || $key == 'notify_user') {
                continue;
            }
    ...
    public function get_notified_form()
    {
    ...
    {
        $obj = static::get_instance($post_id);
        if ($obj->get("post_type") !== static::get_type()) {
            return;
        }
    ...
    public static function add_menu_notification()
    {
        global $submenu ;
